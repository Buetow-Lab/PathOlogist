function p=logrank1(x1,x2,varargin)
...
addOptional(p,'plotflag',1, @(x) isnumeric(x) && isreal(x) && isfinite(x) && isscalar(x) && (x==0 || x==1));
parse(p,x1,x2,varargin{:});
alpha=p.Results.alpha; cflag=p.Results.cflag; plotflag=p.Results.plotflag;
clear p
...
clf
hold on
S1=stairs(t1,T1*100,'r','LineWidth',1.5); %Kaplan-Meier curve for treatment 1
if ~isempty(table12)
    S3=plot(xcg1,ycg1*100,'k+','MarkerSize',12); %Censored data for treatment 1 (if there are)
else
    S3=[];
end

S2=stairs(t2,T2*100,'b','LineWidth',1.5); %Kaplan-Meier curve for treatment 2
if ~isempty(table22)
    S3=plot(xcg2,ycg2*100,'k+','MarkerSize',12,'MarkerFaceColor','k'); %Censored data for treatment 2 (if there are)
end
hold off
...
ylabel('Overall Survival (%)','FontName','Arial','FontSize',12)
xlabel('Times','FontName','Arial','FontSize',12)
if isempty(S3)
    legend([S1 S2],'Treatment 1','Treatment 2')
else
    legend([S1 S2 S3],{'high score','low score','Censored'},'FontName','Arial','FontSize',12)
    % str = {'logrank \itp\rm=0.6045','HR=0.9673, \itp\rm=0.3055'}
    % text(t1(3), T1(1),'HorizontalAlignment','left')
end
set(gca, 'FontSize',12)
...
% disp(cell2table({UL SUL z alpha p txt},'VariableNames',{'UL','Standard_error','z','alpha','two_tailed_p_value','Comment'}))
